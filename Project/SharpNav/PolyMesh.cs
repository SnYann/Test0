...
namespace SharpNav
{
	...
	public class PolyMesh
	{
        ...
        public void PolyMesh2(List<Triangle3> _tris, List<Vector3> _f)// (ContourSet contSet, float cellSize, float cellHeight, int borderSize, int numVertsPerPoly)
        {
            //var verts = new List<PolyVertex>();//ployvertex就是顶点编号
            //var polys = new List<Polygon>();//一个三角网格
            //numVertsPerPoly = _tris.Count;

            //for (int i = 0; i < _tris.Count; i++) verts.Add(new PolyVertex());

            //Polygon p2 = new Polygon(numVertsPerPoly, 1,new RegionId(1));//(numVertsPerPoly, cont.Area, cont.RegionId);
            //for(int i=0;i<_f.Count;i++)
            //{
            //    p2.Vertices = _f[i].ToArray();
            //}
